...
# data ====
data <- read.table("data/proteomics/proteins_phenofile_processed.txt", header = T, sep = "\t")
...
data_analysis <- data %>% 
  select(Idepic, Match_Caseset, Cncr_Caco_Clrt, Sex, Siteclrt, Length_Bld, 
         any_of(columns))
coln <- ncol(data_analysis) - length(proteins) + 1
...
write.table(results, "analysis/002_analysis/proteomics/002_analysis_sex_site-specific_median-followup_removed-2y-followup.txt", 
